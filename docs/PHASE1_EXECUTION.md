...
| **Serializable `GameState`** | **Structurally done** — plain object, no methods, no canvas/DOM; `?dev=1` + `assertGameStateTemplateSerializable()` on the template in `src/dev/assertSerializable.js` | `src/state/GameState.js` | 4 | Med | Runtime `enemies[]._hitThisSwing` / `player._gravityScale` documented; strip in Phase 2 snapshots. |
...
| **Minimal UI + debug overlay** | **Done** — HP, combo, goal line, end overlays, F3 + `` ` `` debug panel, `ROUND` in debug | `src/systems/UI.js`, `src/core/Input.js` | 9 | Low | **Frozen**; F3 or Backquote. |
| **Playable test level** | **Done** — `roundState` + win/lose overlay + `R` / `resetRun()`; goal line while playing | `src/state/GameState.js`, `src/core/Game.js`, `src/core/Input.js`, `src/systems/UI.js` | 10 | Med | **Frozen** for Phase 1. |
